...
public class Network extends Thread{
        ...
        if (context.equals("network"))
          {
            ...
            portID = 0;
            maxNbPackets = 10;
            inComingPacket = new Transactions[maxNbPackets];        //incoming network buffer
            outGoingPacket = new Transactions[maxNbPackets];        //out going network buffer 
    ...
    {	
    	...
    	while (true)
    	{
		// terminate when both the client and server are disconnected
        if(getClientConnectionStatus().equals("disconnected") && getServerConnectionStatus().equals("disconnected")){
            setNetworkStatus("inactive");
            // System.out.println("\n DEBUG: Network.run() - terminating network thread");
            break;
        }

        // if either side is still connected, we need to keep yielding the CPU
        Thread.yield();
