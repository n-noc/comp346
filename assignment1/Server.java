...
public class Server extends Thread{
     ...
     */
     ...
     {   int accIndex;             	/* Index position of account to update */
         ...
         { 
            ...
            //  the line above would continuosly consume the CPU so instead we busy wait while yielding (below)
            while (objNetwork.getInBufferStatus().equals("empty") && !objNetwork.getClientConnectionStatus().equals("disconnected")){
        	 ...
        	 if (!objNetwork.getInBufferStatus().equals("empty"))
        	 {
                ...
                // wait while the output buffer is full
    ...
    {   Transactions trans = new Transactions();
    	...
    	serverStartTime= System.currentTimeMillis();  	/* Record start time of server thread */
        processTransactions(trans);               		
        serverEndTime= System.currentTimeMillis();    	/* Record end time of server thread */

        objNetwork.disconnect(objNetwork.getServerIP());   	/* Disconnect server from network */
